# test_tabs.py - Тест каждой вкладки по отдельности
import sys
import pandas as pd
import numpy as np
...
class TestTabsWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Тест вкладок")
    ...
    def build_all(self):
        """Строит все вкладки"""
        print("🏗️ Построение всех вкладок...")
        ...
        try:
            ...
            # 1. Обзор
            self._build_overview(df)
            print("✅ Обзор построен")
            
            # 2. Категории
            self._build_categories(df)
            print("✅ Категории построены")
            
            # 3. География
            self._build_geography(df)
            print("✅ География построена")
            
            # 4. Мусор
            self._build_trash(df)
            print("✅ Мусор построен")
    ...
    def _build_overview(self, df):
        ...
        if len(cat) > 0:
            ...
            print(f"  Топ категорий: {len(top)}")
    
    def _build_categories(self, df):
        """Строит вкладку Категории"""
        cat = get_by_category(df)
    ...
    def update_geography(self):
        """Обновляет географию при смене типа"""
        if self.filtered_data is not None:
            self._build_geography(self.filtered_data)
    
    def _build_trash(self, df):
        """Строит вкладку Мусор"""
