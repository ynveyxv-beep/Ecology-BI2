# ui/eco_dashboard_window.py - ИСПРАВЛЕННАЯ ВЕРСИЯ (только работающие компоненты)
...
class EcoDashboardWindow(QWidget, LoggerMixin):
    """Экологический дашборд - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    ...
    def _create_overview_widgets(self):
        ...
        # ─── Временной ряд (УПРОЩЁННЫЙ) ──────────────────────────────────
        # Создаём отдельный контейнер для временного ряда
        ts_container = QWidget()
        ts_layout = QVBoxLayout(ts_container)
        ts_layout.setContentsMargins(0, 0, 0, 0)
        
        ts_label = QLabel("📈 Динамика обращений (выполненные)")
    ...
    def _build_time_series(self, df):
        """Строит временной ряд - УПРОЩЁННАЯ ВЕРСИЯ (только выполненные)"""
        ...
        try:
            ...
            # Берём все данные (не только выполненные)
            df_copy = df.copy()
